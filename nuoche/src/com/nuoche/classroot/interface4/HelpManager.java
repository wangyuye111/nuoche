...
public class HelpManager {
	...
	public static int item02 = 20;
	public static int shouyeitem = 8;
	//public static int item02 = 20;// 每页显示多少条
	int n = 2;
	...
	// 翻页 0totlePage：总页数、1totle：总条数、2current：第多少条、3pagenum：当前页数
		public int[] upages(String arg[], int totle) {
			int pagenum = Integer.parseInt(arg[2].equals("0") ? "1" : arg[2]);
			int current = 0;
			int lastcount = 0;
			if (pagenum != 1) {
				current = (pagenum - 1) * item;
		...
		public static int[] pages02(String startPage, int total) {
			int pagenum = Integer.parseInt(startPage.equals("0") ? "1" : startPage);
			int current = 0;
			int lastcount = 0;

			if (pagenum != 1) {
				current = (pagenum - 1) * item02;
			}
			int totlePage = total / item02;
			int totlePageY = total % item02;
			if (totlePageY != 0) {
				totlePage = totlePage + 1;
			}

			if ((current + item02) < total) {
				lastcount = current + item02;
			} else {
				lastcount = total;
			}
	...
	public static boolean isImgUp(String str) {
		...
		mode1List.add("car4s_add");
		mode1List.add("car4s_mod");
		mode1List.add("carshop_add");
		mode1List.add("carshop_mod");
		mode1List.add("credit_add");
		mode1List.add("credit_mod");
		mode1List.add("addshenche");
		mode1List.add("addbaoxian");
		mode1List.add("groupbgood_add");
		mode1List.add("groupbgood_modify");
