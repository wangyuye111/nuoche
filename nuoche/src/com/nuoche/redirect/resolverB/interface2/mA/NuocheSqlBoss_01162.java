...
public class NuocheSqlBoss_01162  extends NuocheSqlManager implements NuocheSqlMFace {
	@Override
	...
	public String searchSqlface(int current, String[] arg) throws SQLException, IOException {
		...
		case "getqrcode":
			if(current==0){
				ressql = "select qrcode from maxqrcode where id=1";
			}else if(current==1){
				ressql = "update maxqrcode set qrcode="+arg[0]+" where id=1";
			}else{
				ressql = "insert into batchcard_info (card_num,car_phone,car_number,car_text,user_openid,card_status) values ('"+ arg[0]
						+ "','"
						+ arg[2]
						+ "','"
						+ arg[3]
		...
		case"sefabunuoche":
			if(current==0){
				ressql="select count(0) from release_table where user_id='"+arg[4]+"'";
			}
			else if(current==1){
				ressql = "select R.user_id,U.nickname,U.head_photo,R.place,R.price,R.type,R.longitude,R.latitude,R.time from release_table as R left join user_data as U on R.User_id =U.id where R.user_id='"+arg[4]+"' order by time desc limit "+arg[5]+","+JyHelpManager.item+"";	
			}
			else if(current==2){
				ressql="select count(0) from release_table";
			}
			else if(current==3){
				ressql = "select R.user_id,U.nickname,U.head_photo,R.place,R.price,R.type,R.longitude,R.latitude,R.time from release_table as R left join user_data as U on R.User_id =U.id order by time desc limit "+arg[5]+","+JyHelpManager.item+"";
			/*
		...
		case "admin_login":
			ressql = "select id from login where username='" + arg[3]
					+ "' and password='" + arg[4] + "'";
			break;
