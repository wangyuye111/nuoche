...
public class NuocheServletInOut_A_01168  extends HttpServlet {

	
	private static final long serialVersionUID = -1204545408335287565L;


	public NuocheServletInOut_A_01168() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		log.send(DataType.specialType, "01168aaa", "vliaoServletInOut_01168_arg:", arg);
...
//		arg = JsonUtil.jsonReceive(request);
		...
		try {
			...
			/*case "A-user-add":
				inOutFace = new NuocheInoutUser_A_01168(arg, request, response);
				inOutFace.addface();
				break;
			case "A-user-mod":
				inOutFace = new NuocheInoutUser_A_01168(arg, request, response);
			...
			case "A-user-delete":
				...
				break;*/
			case "A-boss-add":
				log.send(DataType.basicType, "01168", "修改提现抽成比例-sql", "add");	
				inOutFace = new NuocheInoutBoos_A_01168(arg, request, response);
				inOutFace.addface();
				break;
			case "A-boss-mod":
				log.send(DataType.basicType, "01168", "修改提现抽成比例-sql", "mod");
				inOutFace = new NuocheInoutBoos_A_01168(arg, request, response);
				inOutFace.modface();
				break;
			case "A-boss-search":
				log.send(DataType.basicType, "01168", "修改提现抽成比例-sql", "search");
				inOutFace =new NuocheInoutBoos_A_01168(arg, request, response);
				inOutFace.searchface();
				break;
			case "A-boss-delete":
				log.send(DataType.basicType, "01168", "修改提现抽成比例-sql", "delete");
				inOutFace=new NuocheInoutBoos_A_01168(arg, request, response);
			    inOutFace.deleteface();
				break;
