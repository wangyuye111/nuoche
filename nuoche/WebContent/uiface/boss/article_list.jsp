	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}">
			<tr class="text-c">
				...
				<td>${map['id']}</td>
				<td><img src="<%=path %>/../${map['img']}" width="100px" ></td>
				<td>${map['name']}</td>
				<td>${map['type']}</td>
...
function tiaozhuan1(){
	...
	if(tiaozhuan_input>0 && tiaozhuan_input<=yeshu){
		window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&mode2=ArticleSearch&empty=&page="+tiaozhuan_input;  
	}else{
		alert("页码输入不正确");
	}
}
...
function fresh_page(pageIndex) {
	var name = document.getElementById("searchname").value;
		...
		$.ajax({
			cache:true,
			type:"post",
			url: "<%=path %>/JyServletInOut?a=A-boss-search&b=ArticleLikeSearch&name="+name+"&page="+pageIndex,
			async: true,
			...
			success:function(data){
				...
				for(var i = 0;i<json.length-1;i++){
					content += '<tr class = "text-c">'
							...
							+'<td>'+json[i].id+'</td>'
						    +'<td><img src="<%=path %>/../'+json[i].img+'"  style="width:100px"></td>'
							+'<td>'+json[i].name+'</td>'
...
$("#DataTables_Table_0_previous").click(function() {
	...
	if(bool != 1){
		...
		var page =Number(currentpage-1);
		 ...
		 window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&mode2=ArticleSearch&empty=&page="+page;  
	}else{
		if(currentpage <= 1) {
			alert('当前已经是第一页');
			return;
