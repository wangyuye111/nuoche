...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 提现申请 
...
<div class="page-container">
	<div class="text-c">
		<!-- <div class="mt-20">
			<div class="text-c">
					地区：
				<input type="text" class="input-text" style="width:250px"  placeholder="地区，如：广东省" id="searchtxt" name="searchtext">	
				<span>性别:</span>
				...
				<button type="submit" class="btn btn-success radius" id="searchbtn" name=""><i class="Hui-iconfont"></i>搜车位</button>
			</div>	
		</div> -->
		...
		<tbody id="list-content">
			<c:forEach var = "map" items="${reList }" varStatus="status">
				<tr class="text-c">
					...
					<td>${map['nickname']}</td>  
					<td><a  style="color:#1a69e2"   href="javascript:;" onclick="system_category_add('订单明细','<%=path %>/JyServletInOut?a=A-boss-search&b=withdraw_info&id=${map['id']}&page=1&tojj=jsp','1000','600')"  >${map['order_nums']}(笔)</a></td>
					<td>${map['withdraw_money']}元</td>
					<td>${map['withdraw_time']}</td>
					<td>${map['withdraw_status']}</td>
					<td>
					<c:if test="${map['withdraw_status'] == '提现成功' }">提现成功</c:if>
				    <c:if test="${map['withdraw_status'] == '申请中' }"><input  type="button"  class="btn btn-warning radius" onclick="download('${map['id']}')" value="确认"></c:if>
...
function download(id){
	...
	$.ajax({
		cache: true,
		type: "POST",
		url:"<%=path %>/JyServletInOut?mode=A-boss-search&mode2=withdraw_down&empty="+id,
		async: true,
...
function fresh_page(pageIndex){
	...
	window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&mode2=withdraw_list&empty=&page="+pageIndex+"&tojj=jsp";  
	<%-- /* 获取输入的性别 */
	/* 获取输入的起始时间和结束时间 */
	/* 固定死判断条件，如性别，时间段：开始时间---结束时间 */
	$.ajax({
		...
		url:"<%=path%>/ar?p0=A-boss-search&p1=user_list&p2="+pageIndex+"&p3=tojson&p4="+mil_id,
		url: "<%=path %>/JyServletInOut?a=A-boss-search&b=withdraw_list&name=&page="+pageIndex+"&tojj=json",
		async: true,
		error: function(request) {
			alert("提交失败 ");
		},
			...
			for(var i = 0; i < json.length-1; i++) {
				...
				<td>${map['time']}</td> */
				 ...
				 content +='<tr class="text-c">'
					...
					+'<td><a  style="color:#1a69e2" href="'+paht+'/JyServletInOut?a=A-boss-search&b=withdraw_info&id='+json[i].id+'&page=1&tojj=jsp" >'+json[i].order_nums+'(笔)</a></td>'
					+'<td>'+json[i].withdraw_money+'元</td>'
					+'<td>'+json[i].withdraw_time+'</td>'
					+'<td>'+json[i].withdraw_status+'</td>'
					<td><input  type="button"  class="btn btn-warning radius" onclick="download('${map['id']}')" value="确认"></td>
