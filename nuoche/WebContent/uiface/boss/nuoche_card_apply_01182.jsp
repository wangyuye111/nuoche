...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 邮寄挪车卡 
...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
		<!--User_id,Name,Phone,Address,Number,Mail_status,Code_id,Is_pay,Apply_time  -->
			<tr class="text-c">
				<th width=20>序号</th>
				<th width="20">用户ID</th>
				<th width="40">姓名</th>
				<th width="40">手机号</th>
				<th width="40">地址</th>
				<th width="40">挪车卡申请数量</th>
				<th width="40">邮寄状态</th>
				<th width="40">挪车卡二维码</th>
				<th width="40">是否支付</th>
				<th width="40">邮寄时间</th> 				
			</tr>
		</thead>
		<tbody id="list-content">
			...
			<!--User_id,Name,Phone,Address,Number,Mail_status,Code_id,Is_pay,Apply_time  -->
				<tr class="text-c">
					...
					<td  id="dingdan${map['id']}">
					<c:if test="${map['send_number'] == '' && map['is_pay'] == '1'  }"><input  type="button"  class="btn btn-warning radius" onclick="youji('${map['id']}')" value="邮寄"></c:if>
					<c:if test="${map['send_number'] == '' && map['is_pay'] == '0'  }">不可邮寄</c:if>
					<c:if test="${map['send_number'] != '' && map['is_pay'] == '1'  }">${map['send_number']}</c:if>
					</td>
					<td>
					<c:if test="${map['code_phone'] == '' }"></c:if>
					<c:if test="${map['code_phone'] != '' }"><img style="width:100px;height:100px" src="${map['code_phone']}" /></c:if>
					</td>
					
					<td>
					<%-- ${map['is_pay']} --%>
					<c:if test="${map['is_pay'] == '1' }">已支付</c:if>
					<c:if test="${map['is_pay'] != '1' }">未支付</c:if>
					</td>
					<td>${map['apply_time']}</td>
				</tr>
			</c:forEach>
	...
	if(number==null){
		...
		if(number !=''){
			$.ajax({
				...
				url:"<%=path%>/ar?p0=A-boss-search&p1=apply_card_youji&p2="+id+"&p3="+number,
...
function fresh_page(pageIndex){
	...
	$.ajax({
		...
		type: "POST",
		<%-- url: "<%=path%>/ar?p0=A-boss-search&p1=fabu&p2="+pageIndex+"&p3=&p4=&p5="+sele_condition+"&p6=tojson&p7="+mil_id , --%>
		url:"<%=path%>/ar?p0=A-boss-search&p1=apply_card&p2="+pageIndex+"&p3=tojson&p4="+mil_id,
		...
		success: function(data){
			...
			for(var i = 0; i < json.length-1; i++) {
				...
				<td>${map['type']}</td>
				<td>${map['states']}</td> */
				if(json[i].code_phone==''){
					str3='<td>已支付</td>';
				}else{
					str3='<td><img style="width:100px;height:100px" src="'+json[i].code_phone+'" /></td>';
				}
				
				if(json[i].send_number=='' && json[i].is_pay=='1'){
					str4='<td  id="dingdan'+json[i].id+'"><input  type="button"  class="btn btn-warning radius" onclick="youji('+json[i].id+')" value="邮寄"></td>';
				}else if(json[i].send_number=='' && json[i].is_pay=='0'){
					str4='<td   id="dingdan'+json[i].id+'">不可邮寄</td>';
				}else if(json[i].send_number!='' && json[i].is_pay=='1'){
					str4='<td   id="dingdan'+json[i].id+'">'+json[i].send_number+'</td>';
				}
