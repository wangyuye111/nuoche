...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><!-- <a href="javascript:;" onclick="deletes()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> --> <a href="javascript:;" onclick="system_category_add('文章添加','<%=path %>/JyServletInOut?a=A-boss-search&b=article_add','1000','600')"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加文章</a></span> </div>
	 --%><div class="mt-20">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}">
			<tr class="text-c">
				...
				<td>${map['car_phone']}</td>
				<td> <img src="https://vipz.top/img/qrcodeimg/${map['card_num']}.jpg" style="width:100px;height:100px" />  </td>
				<%-- <td>${map['type']}</td>
...
function tiaozhuan1(){
	...
	if(tiaozhuan_input>0 && tiaozhuan_input<=yeshu){
		window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&b=batchcardinfo&id=${reList[0]['batchcard_num']}&page="+tiaozhuan_input+"&tojj=jsp";  
	}else{
		alert("页码输入不正确");
	}
}
...
function fresh_page(pageIndex) {
	window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&b=batchcardinfo&id=${reList[0]['batchcard_num']}&page="+pageIndex+"&tojj=jsp";  
	...
	<%-- var name = "";
		...
		$.ajax({
			cache:true,
			type:"post",
			url: "<%=path %>/JyServletInOut?a=A-boss-search&b=batchcardinfo&id=${reList[0]['batchcard_num']}&page="+pageIndex+"&tojj=json",
			async: true,
			...
			success:function(data){
				...
				for(var i = 0;i<json.length-1;i++){
					content += '<tr class = "text-c">'
							...
							+'<td>'+json[i].id+'</td>'
							+'<td>'+json[i].card_num+'</td>'
							+'<td>'+json[i].car_number+'</td>'
							+'<td>'+json[i].car_phone+'</td>'
							+'<td><img src="https://vipz.top/img/qrcodeimg/'+json[i].agentphone+'.jpg"    />  </td>'
...
$("#DataTables_Table_0_previous").click(function() {
	...
	var page =Number(currentpage+1);
	 ...
	 window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&b=batchcardinfo&id=${reList[0]['batchcard_num']}&page="+page+"&tojj=jsp"; 
	...
	<%-- if(bool != 1){
		...
		var page =Number(currentpage-1);
		 ...
		 window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&b=batchcardinfo&id=${reList[0]['batchcard_num']}&page="+page+"&tojj=json";  
	}else{
		if(currentpage <= 1) {
			alert('当前已经是第一页');
			return;
