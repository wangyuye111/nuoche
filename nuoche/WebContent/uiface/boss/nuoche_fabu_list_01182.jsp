...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 发布管理 
	<span class="c-gray en">&gt;</span> 发布挪车 
		...
		<tbody id="list-content">
			<c:forEach var = "map" items="${reList }" varStatus="status">
				<tr class="text-c">
					...
					<td>${map['nickname']}</td>
					<td>${map['place']}</td>
					<td>${map['price']}元/小时</td>
					<td>${map['paymoney']}元</td>
					<td>${map['cw_states']}</td>
					<td>${map['time']}</td>
					<td>${map['end_timef']}</td>
					<%-- <td>${map['time']}</td>
...
function fresh_page(pageIndex){
	...
	$.ajax({
		...
		url:"<%=path%>/xnc?p0=A-boss-search&p1=sefabunuoche&p2="+pageIndex+"&p3=tojson&p4="+mil_id,
		async: true,
			...
			for(var i = 0; i < json.length-1; i++) {
				...
				<td>${map['time']}</td> */
				 ...
				 content +='<tr class="text-c">'
					...
					+'<td>'+json[i].place+'</td>'
					+'<td>'+json[i].price+'元/小时</td>'
					+'<td>'+json[i].paymoney+'元</td>'
					+'<td>'+json[i].cw_states+'</td>'
					+'<td>'+json[i].time+'</td>'
					+'<td>'+json[i].end_timef+'</td>'
