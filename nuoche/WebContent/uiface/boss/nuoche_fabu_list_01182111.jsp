...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
			<!-- R.ID,U.Nickname,U.Head_photo,R.Place,R.Price,R.Type,
			longitude,R.latitude,R.Time-->
				<th width=20>序号</th>
				<th width="20">ID</th>
				<th width="40">头像</th>
				<th width="40">昵称</th>
		...
		<tbody id="list-content">
			<c:forEach var = "list" items="${list }" varStatus="status">
				<tr class="text-c">
		<!-- R.User_id,U.Nickname,U.Head_photo,R.Place,R.Price,R.Type,
			longitude,R.latitude,R.Time-->
					<td>${status.count}</td>
					<td>${list['user_id']}</td>
					<td><img alt="" src="${list['head_photo']}" style="width:80px"></td>
					<td>${list['nickname']}</td>
					<td>${list['place']}</td>
					<td>${list['price']}</td>
					<td>${list['type']}</td>
