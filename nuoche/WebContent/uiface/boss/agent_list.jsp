...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 代理商管理 <span class="c-gray en">&gt;</span> 代理商申请列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}">
			<tr class="text-c">
				...
				<td>${map['id']}</td>
				<td>${map['agentname']}</td>
				<td>${map['agentphone']}</td>
				<td>${map['agent_wx']}</td>
				<td>${map['agent_email']}</td>
				<td>${map['agent_address']}</td>
				<%-- <td>${map['type']}</td>
...
function tiaozhuan1(){
	...
	if(tiaozhuan_input>0 && tiaozhuan_input<=yeshu){
		window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&mode2=agent_list&empty=&page="+tiaozhuan_input+"&tojj=jsp";  
	}else{
		alert("页码输入不正确");
	}
}
...
function fresh_page(pageIndex) {
	var name = "";
		...
		$.ajax({
			cache:true,
			type:"post",
			url: "<%=path %>/JyServletInOut?a=A-boss-search&b=agent_list&name="+name+"&page="+pageIndex+"&tojj=json",
			async: true,
			...
			success:function(data){
				...
				for(var i = 0;i<json.length-1;i++){
					content += '<tr class = "text-c">'
							...
							+'<td>'+json[i].id+'</td>'
							+'<td>'+json[i].agentname+'</td>'
							+'<td>'+json[i].agentphone+'</td>'
							+'<td>'+json[i].agent_wx+'</td>'
							+'<td>'+json[i].agent_email+'</td>'
							+'<td>'+json[i].agent_address+'</td>'
...
$("#DataTables_Table_0_previous").click(function() {
	...
	if(bool != 1){
		...
		var page =Number(currentpage-1);
		 ...
		 window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&mode2=agent_list&empty=&page="+page+"&tojj=json";  
	}else{
		if(currentpage <= 1) {
			alert('当前已经是第一页');
			return;
