...
function fresh_page(pageIndex){
	...
	<%-- //window.location.href = "<%=path %>/JyServletInOut?mode=A-boss-search&mode2=withdraw_info&empty=&page="+pageIndex+"&tojj=jsp";   --%>
	 /* 获取输入的性别 */
	/* 获取输入的起始时间和结束时间 */
	/* 固定死判断条件，如性别，时间段：开始时间---结束时间 */
	$.ajax({
		cache: true,
		type: "POST",
		url: "<%=path %>/JyServletInOut?a=A-boss-search&b=withdraw_info&name=&page="+pageIndex+"&tojj=json",
		async: true,
